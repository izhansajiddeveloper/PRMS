...
if (isset($_GET['delete']) && isset($_GET['id'])) {
    ...
    $get_user = "SELECT user_id FROM staff WHERE id = ?";
    ...
    if ($staff) {
        ...
        try {
            ...
            mysqli_commit($conn);
            setFlashMessage("Receptionist deleted successfully!", "success");
        } catch (Exception $e) {
            mysqli_rollback($conn);
            setFlashMessage("Failed to delete receptionist!", "error");
        }
    }
...
if (isset($_GET['toggle']) && isset($_GET['id'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        setFlashMessage("Receptionist status updated successfully!", "success");
    } else {
        setFlashMessage("Failed to update receptionist status!", "error");
    }
...
$where_clause = " WHERE u.role_id = 3 ";
if ($search) {
    $where_clause .= " AND (u.name LIKE '%$search%' OR s.position LIKE '%$search%' OR u.email LIKE '%$search%' OR u.phone LIKE '%$search%') ";
}

// Get total records for pagination
$total_query = "SELECT COUNT(*) as total FROM staff s 
...
// Fetch staff (receptionists) with search and pagination
$query = "SELECT s.*, u.id as user_id, u.name, u.email, u.phone, u.status, u.created_at 
          FROM staff s 
          JOIN users u ON s.user_id = u.id 
          $where_clause
...
<div class="flex-1 overflow-y-auto hide-scrollbar bg-gray-50">
    <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Receptionists Management</h1>
                <p class="text-gray-600 mt-1">Manage all reception staff in the system</p>
            </div>
            <a href="create.php" class="bg-gradient-to-r from-blue-500 to-green-500 text-white px-5 py-2 rounded-lg hover:shadow-lg transition">
                <i class="fas fa-plus mr-2"></i>Add New Receptionist
            </a>
        </div>
                        ...
                        <?php if (mysqli_num_rows($result) > 0): ?>
                            <?php while ($staff = mysqli_fetch_assoc($result)): ?>
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-4 py-4 text-sm text-gray-800"><?php echo $staff['id']; ?></td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-3">
                                                <p class="text-sm font-medium text-gray-800 whitespace-nowrap"><?php echo htmlspecialchars($staff['name']); ?></p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="px-2 py-1 text-xs rounded-full bg-purple-100 text-purple-800 whitespace-nowrap">
                                            <?php echo htmlspecialchars($staff['position']); ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="px-2 py-1 text-xs rounded-full whitespace-nowrap 
                                            <?php echo $staff['shift'] == 'Morning' ? 'bg-yellow-100 text-yellow-800' : ($staff['shift'] == 'Evening' ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'); ?>">
                                            <?php echo htmlspecialchars($staff['shift']); ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap"><?php echo htmlspecialchars($staff['email']); ?></td>
                                    <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap"><?php echo htmlspecialchars($staff['phone']); ?></td>
                                    <td class="px-4 py-4">
                                        <span class="px-2 py-1 text-xs rounded-full whitespace-nowrap 
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap"><?php echo date('d M Y', strtotime($staff['created_at'])); ?></td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <a href="?toggle&id=<?php echo $staff['user_id']; ?>"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg <?php echo $staff['status'] == 'active' ? 'bg-yellow-50 text-yellow-600 hover:bg-yellow-600' : 'bg-green-50 text-green-600 hover:bg-green-600'; ?> hover:text-white shadow-sm transition-all"
                                                title="<?php echo $staff['status'] == 'active' ? 'Deactivate' : 'Activate'; ?> Account">
                                                <i class="fas <?php echo $staff['status'] == 'active' ? 'fa-ban' : 'fa-check-circle'; ?> text-xs"></i>
                                            </a>

                                            <a href="javascript:void(0)"
                                                onclick="confirmDelete(<?php echo $staff['id']; ?>, '<?php echo htmlspecialchars($staff['name']); ?>')"
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center text-gray-500">
                                    <i class="fas fa-headset text-4xl mb-3 opacity-50"></i>
                                    <p>No receptionists found</p>
                                    <a href="create.php" class="text-blue-600 hover:underline mt-2 inline-block">Add a receptionist</a>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
...
<script>
    function confirmDelete(id, name) {
        if (confirm(`Are you sure you want to delete receptionist "${name}"? This action cannot be undone.`)) {
